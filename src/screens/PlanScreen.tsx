...
export const PlanScreen: React.FC<MainTabScreenProps<'Plan'>> = ({ navigation }) => {
  ...
  useEffect(() => {
    if (dataLoaded && allDishes.length > 0 && !hasAutoGenerated) {
      setHasAutoGenerated(true);
      generate();
    }
  }, [dataLoaded, allDishes.length, hasAutoGenerated]);
  ...
  }, [plan, householdId, user, allDishes, addMeal, navigation]);
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <View style={styles.weekNav}>
            <IconButton icon="chevron-left" size={24} onPress={() => { setWeekOffset((w) => Math.max(0, w - 1)); setPlan([]); setHasAutoGenerated(false); }} disabled={weekOffset === 0} />
            <Text style={styles.weekLabel}>{dateRangeLabel}</Text>
            <IconButton icon="chevron-right" size={24} onPress={() => { setWeekOffset((w) => w + 1); setPlan([]); setHasAutoGenerated(false); }} />
          </View>
          <Text style={styles.subtitle}>
            {plan.length > 0 ? 'Review and accept your plan' : 'Generate a plan for this week'}
          </Text>
        </View>
