...
- **BLOCKERS FOUND (Sameer must act):** (a) **EU DSA trader status** — App Store Connect banner: must declare trader/non-trader status or apps get pulled from EU. (b) **App record not yet created** — ASC Apps list is empty; "Sofra" name availability UNVERIFIED (could be taken globally). (c) **The signed build itself needs Sameer's Apple creds** — I can't enter his Apple ID/2FA (prohibited). He runs `eas build -p ios --profile production` in his terminal (interactive Apple login) OR sets up an App Store Connect API key (.p8) for non-interactive. Then `eas submit -p ios` (can auto-create the app record) → ASC App Privacy label + metadata + screenshots → submit for review.
